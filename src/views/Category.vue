    ...
    <div class="container mx-auto px-4 py-8">
      ...
      <header class="max-w-4xl mx-auto mb-12">
        ...
        <nav class="flex items-center space-x-2 text-sm text-gray-500 dark:text-gray-400 mb-6">
          ...
          <span>/</span>
          <span class="text-gray-700 dark:text-gray-300">{{ getCategoryDisplayName(category) }}</span>
        </nav>
              ...
              <div class="flex items-center justify-between">
                <div class="flex flex-wrap gap-2">
                  <span 
                    ...
                    :key="tag"
                    class="px-2 py-1 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-xs rounded cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors"
      ...
      <div v-else class="max-w-2xl mx-auto text-center py-12">
        <div class="mb-6">
          ...
          <p class="text-gray-600 dark:text-gray-300">
            {{ getCategoryDisplayName(category) }} 分类下还没有文章
          </p>
        </div>
        <router-link 
          to="/"
...
const { posts, loadPosts, getPostsByCategory } = useBlogPosts()
...
const getCategoryDescription = (category) => {
  const descriptionMap = {
    'tech': '探索前沿技术，分享开发经验',
    'tutorials': '详细的技术教程和实践指南',
    'projects': '项目实战和架构设计分享',
    'life': '生活感悟和个人思考',
    'thoughts': '技术思考和行业观察'
  }
  return descriptionMap[category] || '分类文章列表'
}
